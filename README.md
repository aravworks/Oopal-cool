...
| **Therapist Finder** | Google News RSS + Gemini analysis; finds real, named therapists in real news coverage, no directory scraping |
...
#### F8 — Therapist Finder (Google News + Gemini + Map)
...
cryptographic challenges, TLS-fingerprint blocking) that blocks plain
`requests` scraping *and* real headless-browser automation — confirmed by
direct testing. Scraping Google Search/Maps results directly is against
Google's ToS and gets blocked even harder. So instead of scraping a
directory site, this searches Google News' public RSS feed for real
coverage naming real therapists (awards, recognition, features).

**Backend flow**:
1. Fetch user's latest diagnosis from Supabase (condition drives the query)
2. Query Google News RSS (`news.google.com/rss/search`) with condition- and
   city-specific queries (e.g. `"Delhi" anxiety therapist recognition award`)
3. Feed the real headlines to Gemini: identify which ones actually name a
   specific therapist/psychologist/clinic (not generic awareness pieces),
   extract only what's stated (never fabricate a fee, phone, or rating not
   in the source), and score relevance to the user's condition
4. Geocode the city via OpenStreetMap's Nominatim for an approximate map pin
5. Return up to 5 results with a link back to the source article; empty
   list (no fake fallback data) if nothing genuine turns up
6. Display on an OpenStreetMap/Leaflet map with approximate city-level pins

**Data returned per therapist**:
- Name (or clinic/center name if no individual named)
- Why they're notable (extracted from the real headline)
- Relevance to the user's condition (Gemini's reasoning, grounded in the article)
- Source publication + link to the original article
- Approximate city-level map pin (not an exact clinic address — we don't have that data)
...
| `requests` | Google News RSS fetch + Nominatim geocoding |
...
│   ├── scraper.py                  # Therapist finder (Google News RSS + Gemini)
